...
def render_tab1_baseline():
    """
    Renders the Baseline interface. Fetches active language configuration 
    directly from session state and updates global storage with data sets.
    """
    # Pull the active translation layer from global memory
    ...
    with col_input:
        ...
        # Save structural attributes to session state
        ...
        if uploaded_file:
            try:
                ...
                if len(selected_dates) == 2:
                    ...
                    # Persist calculation baseline for downstream tabs
                    ...
                    fig_load.update_layout(height=400, yaxis_title="kW", margin=dict(t=10, b=10))
                    st.plotly_chart(fig_load, use_container_width=True)
                    
            except Exception as e:
                st.error(f"Error processing baseline data: {e}")
        else:
            st.info("Please upload a CSV file to display data profiles.")