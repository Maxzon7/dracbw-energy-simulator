...
def render_tab2_scenarios():
    ...
    with col_input:
        ...
        # --- BATCHING FORM WITH MEMORY SYNC ---
        ...
        with st.form(key=f"sim_form_{scenario_mode}"):
            ...
            if scenario_mode == "Solar PV Only":
                ...
                with st.expander("Configure Battery Storage", expanded=True):
                    # INJECTION: Wir übergeben das Schmierblatt an die Batterie
                    params = render_battery_ui(
                        scenario_id=selected_baseline, 
            ...
            else: # Combined Mode (Die Kaskade)
                ...
                with st.expander("Configure Battery Storage", expanded=False):
                    # Für den Combined-Mode greifen wir auf das geschachtelte Dictionary zu falls vorhanden
                    ...
                    params['battery'] = render_battery_ui(
                        scenario_id=f"{selected_baseline}_c_bat", 
                        default_grid_limit=grid_limit,
            ...
            # FEEDBACK LOOP: Speichere alle gemachten Eingaben in Tab 2 sofort ab!
            if run_sim:
