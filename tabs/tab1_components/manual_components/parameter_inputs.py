...
def render_scenario_selector(active_scenario: str) -> tuple:
    """
    Renders the dropdown to load existing templates from the vault.
    Returns the selected template name and its parameter dictionary.
    """
...
def render_all_input_fields(p: dict, active_scenario: str, selected_template: str) -> dict:
    ...
    Returns a consolidated dictionary of all user selections.
    """
    ...
    with col2:
        ...
        enable_noise = st.toggle("Enable realistic load fluctuations", value=p.get('enable_noise', False), key=f"noise_toggle_{active_scenario}")
    ...
    use_custom_months = st.checkbox("Enable custom logic per month", value=p.get('use_custom_months', False), key=f"cust_month_toggle_{active_scenario}")
    monthly_configs = {}
    ...
    # Pack everything into a structured dictionary to pass back to the orchestrator
    return {
