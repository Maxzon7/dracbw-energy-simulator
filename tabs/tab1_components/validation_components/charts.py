...
def render_multi_resolution_charts(df: pd.DataFrame, params: dict, statistical_anomalies: pd.DataFrame):
    ...
    with tab_full:
        fig_full = go.Figure()
        fig_full.add_trace(go.Scatter(x=df['timestamp'], y=df['consumption_kw'], mode='lines', line=dict(color=col_raw, width=1), name=f'Client Load'))
    ...
    with tab_month:
        ...
        fig_month.add_trace(go.Scatter(x=sub_df['timestamp'], y=sub_df['consumption_kw'], mode='lines', line=dict(color=col_raw, width=1.5), name='Client Load'))
    ...
    with tab_week:
        ...
        fig_week.add_trace(go.Scatter(x=weekly_profile['synthetic_timestamp'], y=weekly_profile['consumption_kw'], mode='lines', line=dict(color=col_raw, width=2), name='Mean Weekly Footprint'))
