...
# --- Helper class to restore state on import operations ---
...
def render_manual_builder(active_scenario: str, is_edit_mode: bool, base_p: dict):
    ...
    # Condition A: User explicitly clicked the generation trigger button
    ...
    # Condition B: Keep the current generated data visible during secondary form submissions (e.g., Save Button)
    elif st.session_state.get('manual_df_ready', False) and 'filtered_data' in st.session_state:
        df = st.session_state['filtered_data']
        
    # Condition C: Fallback to existing vault entity if working in explicit historical edit mode
    elif is_edit_mode and active_scenario in st.session_state.get('scenario_vault', {}):
        df = st.session_state['scenario_vault'][active_scenario].get("df")

    # 5. Route to Validation funnel if a dataset is actively present
